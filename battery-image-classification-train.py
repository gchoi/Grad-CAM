"""
@Description: Train the model for classification of battery images
@Date: 5 May., 2020
@Author: Alex Choi
...
baseModel = VGG16(weights="imagenet",
                  include_top=False,
                  input_tensor=Input(shape=(RESIZE_H, RESIZE_W, 3)))
...
for i in range(len(model.layers)):
    print(f'Layer Name: {model.layers[i].name}, Trainable: {model.layers[i].trainable}')
...
plt.title("Training Loss and Accuracy on Battery Dataset")
plt.xlabel("Epoch #")
...
print("[INFO] saving Battery Classification model...")
...
# Show a nicely formatted classification report
print(classification_report(trainY.argmax(axis=1), predIdxs, target_names=le.classes_))
